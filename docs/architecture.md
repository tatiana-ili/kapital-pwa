...
Этап 1 включает рабочий Dashboard, экран Transactions, PWA-оболочку, светлую/тёмную темы, Supabase-клиент, PostgreSQL-схему с RLS и детерминированный demo dataset. CSV/XLSX/PDF-парсинг, правила категоризации и остальные продуктовые разделы остаются следующими отдельными итерациями.
...
6. Demo-режим не является авторизованным production-режимом. После задания Supabase env приложение должно требовать сессию; RLS остаётся обязательной последней линией защиты.
7. PWA-кеш не должен сохранять ответы с приватными данными без стратегии. Текущий service worker кеширует shell; перед подключением реальных API правила кеширования нужно ограничить статическими ресурсами.
